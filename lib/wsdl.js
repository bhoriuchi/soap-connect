...
module.exports = function(env) {
	...
	function _equals(value1, value2, i) {
		if ((value1 === value2) || (i === true && _.lowerCase(value1) === _.lowerCase(value2))) {
			return true;
		}
		return false;
	}
	...
	function attrName(attr) {
		return attr.name || attr.nodeName || attr.localName || attr.tagName;
	}
	...
	function expandChildren(element, childSchema) {
		_.forEach(element.childNodes, function(child) {
			var childType = _.lowerCase(attrName(child)).replace(/\s+/g, '');
	...
	wsdlTypes.element = function(element, childSchema) {
		var name = element.getAttribute('name');
		var type = element.getAttribute('type');
		if (name && type) {
			childSchema.fields[name] = type;
		}
		else {
			...
			_.forEach(_.uniq(_.values(de._nsMap)), function(ns) {
				_.forEach(doc.getElementsByTagNameNS(ns, 'schema'), function(tag) {					
					...
					_.forEach(tag.childNodes, function(child) {
						...
						var childType = attrName(child);
						
						if (iequals(childType, ['element', 'complexType'])) {
							...
							schema[childName].fields = schema[childName].fields || {};
							
							if (_.has(wsdlTypes, childType)) {
								wsdlTypes[_.lowerCase(childType)](child, schema[childName]);
							}
