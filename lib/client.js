...
module.exports = function(env) {
	...
	fnWSDL.definitions = function(tag, meta, ns, opt, loaded, emitter) {
		...
		_meta[tns] = {};
		_meta[tns][opt.$$('nsMap')] = _.get(tag, '_nsMap');
		_.merge(_meta[tns], fn.processChildren(tag, meta, tns, opt, loaded, emitter));
		return _meta;
	};
	...
	return function(mainWSDL, options) {
		...
		return buildClient(mainWSDL, options).then(function(metadata) {
			console.log(JSON.stringify(metadata['urn:vim25'], null, '  '));
			wsdl._meta = metadata;
			return wsdl;
		});
	};
